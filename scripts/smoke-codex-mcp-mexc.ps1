param(
  [string]$ServerName = 'trading_mcp_mexc_local'
)
...
$outputPath = Join-Path $artifactsDir 'codex-mcp-mexc-smoke-last-message.txt'
...
if (-not $message) {
  throw "Codex MEXC smoke test produced an empty final message."
}

if ($message -eq 'MCP_UNAVAILABLE') {
  throw "Codex MEXC smoke test could not access the configured MCP server."
}

Write-Host "Codex MEXC MCP smoke output:"
Write-Host $message
