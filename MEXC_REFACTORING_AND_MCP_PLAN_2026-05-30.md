...
По состоянию на 2026-06-01 в репозитории уже сделано следующее:

- Этапы рефакторинга 1-2 выполнены:
  - есть generic MCP runtime в `src/core/mcp/server.ts`
  - есть generic exchange runtime contract в `src/core/exchange-runtime.ts`
  - текущий Bybit runtime вынесен в `src/exchanges/bybit/runtime.ts`
- Для MEXC реализован первый рабочий `Spot REST MVP`:
  - `src/exchanges/mexc/config.ts`
  - `src/exchanges/mexc/auth/spot-signing.ts`
  - `src/exchanges/mexc/rest/spot-client.ts`
  - `src/exchanges/mexc/tools/**`
  - `src/exchanges/mexc/runtime.ts`
  - `src/mexc.ts`
  - `scripts/mexc-smoke.ts`
...
- M2. spot auth layer
- M3. spot REST client
- M4. базовую response/error normalization
- M7. минимальный tool taxonomy для `market`, `account` и `trade`

Дополнительно по состоянию на 2026-05-31:

- реализованы spot trade endpoints:
  - `createTestOrder`
  - `createOrder`
  - `cancelOrder`
  - `cancelAllOrders`
  - `getOrder`
- `scripts/mexc-smoke.ts` теперь проверяет не только public/account endpoints, но и безопасный signed write-path через `POST /api/v3/order/test`
- добавлен authenticated account endpoint:
  - `getMyTrades`
- реализован первый рабочий `Spot WebSocket MVP`:
  - `src/exchanges/mexc/ws/spot-client.ts`
  - `src/exchanges/mexc/tools/websocket/**`
  - `scripts/mexc-ws-smoke.ts`
- live `WS` smoke уже покрывает:
  - aggregated book ticker
  - partial orderbook depth
  - aggregated public trades
- реализован `Futures REST read-only` слой:
  - `src/exchanges/mexc/auth/futures-signing.ts`
  - `src/exchanges/mexc/rest/futures-client.ts`
  - `src/exchanges/mexc/tools/futuresMarket/**`
  - `src/exchanges/mexc/tools/futuresAccount/**`
- реализован первый рабочий `Futures trade` write-layer:
  - `src/exchanges/mexc/tools/futuresTrade/**`
  - live-confirmed paths:
    - `order/create`
...
- реализованы расширенные read-only Spot groups:
  - `capital`
  - `subAccount`
  - `rebate`
  - дополнительные `market` и `account` query tools
- добавлен отдельный live smoke для нового read-only слоя:
  - `scripts/mexc-readonly-smoke.ts`
- добавлен отдельный live write smoke:
  - `scripts/mexc-write-smoke.ts`
- фактически `MEXC` runtime теперь поднимает `80` tools и уже даёт агенту прямой доступ к:
  - spot open orders/history
  - futures active open orders
  - futures trigger / plan orders
  - futures stop orders
  - futures limit order placement/cancellation
  - futures trigger order placement/cancellation
  - futures TP/SL update/cancel flows

Что пока ещё не сделано:

- subscription abstraction
- private `listenKey` user streams
- generic subscription tools for MEXC
- exchange-aware codegen inventory
...
Так как MEXC официально не предоставляет sandbox, базовая smoke strategy должна быть read-only на старте, а write smoke нужно держать отдельным и с агрессивным cleanup.
...
- futures limit order:
  - place
  - confirm in open orders
  - cancel by `externalOid`
- futures trigger order:
  - place with `takeProfitPrice` and `stopLossPrice`
  - confirm in trigger list
  - cancel and confirm state transition
- spot real-order path:
  - optional best-effort only
  - may be skipped on accounts without free spot balance
...
- write smoke держать отдельной командой и использовать только после явного запроса;
...
- шаги `1-2` выполнены для `read-only REST`
- шаг `4` выполнен существенно дальше первоначального плана:
  - futures market read-only tools готовы
  - futures account/order query tools готовы
  - futures write-capable trade tools готовы для основного order lifecycle
  - live mainnet smoke подтверждает:
    - futures limit place/cancel
    - futures trigger place/cancel
    - trigger orders с `takeProfitPrice` / `stopLossPrice`
  - futures WS ещё не делался
...
- write-path testing остаётся рискованным;
- отдельный write smoke возможен только c явным cleanup и без sandbox;
